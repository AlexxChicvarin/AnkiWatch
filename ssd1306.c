...
void ssd1306_command(uint8_t c)
{
    ret_code_t ret;
    uint8_t data[] = {0x00, c};
    ret = nrf_drv_twi_tx(&m_twi_master, _i2caddr, &(data[0]), 2, false);
    if (ret != NRF_SUCCESS) {
 //       putchar('w');
        return;
    }
...
void ssd1306_begin(uint8_t vccstate, uint8_t i2caddr, bool reset)
{
    ...
    ssd1306_command(SSD1306_SETPRECHARGE);                  // 0xd9   !!!!!! default 0x22 or 0xF1  !!!!!!!
//    if (vccstate == SSD1306_EXTERNALVCC) {
//        ssd1306_command(0x22);
//    }
//    else {
...
//    }
    ...
    ssd1306_command(SSD1306_SETVCOMDETECT);                 // 0xDB    !!!!! default 0x40 !!!!!!
    ssd1306_command(0x00);
    ssd1306_command(SSD1306_DISPLAYALLON_RESUME);           // 0xA4
    ssd1306_command(SSD1306_NORMALDISPLAY);                 // 0xA6
#endif
...
void ssd1306_draw_char(int16_t x, int16_t y, uint8_t c, uint16_t color, uint16_t bg, uint8_t size)
{
    ...
    for (int8_t i = 0; i < 6; i++ ) {
        ...
        GFXglyph *gliph = &(((GFXglyph *)pgm_read_pointer(&gfxFont->glyph))[c]);
        uint8_t *bitmap = (uint8_t *)pgm_read_pointer(&gfxFont->bitmap);

        uint16_t bo = pgm_read_word(&gliph->bitmapOffset);
        uint8_t  w  = pgm_read_byte(&gliph->width);
        uint8_t  h  = pgm_read_byte(&gliph->height);
        int8_t   xo = pgm_read_byte(&gliph->xOffset);
        int8_t   yo = pgm_read_byte(&gliph->yOffset);
        ...
        for(yy=0; yy<h; yy++) {
            for(xx=0; xx<w; xx++) {
                ...
                if(bits & 0x80) {
                    if(size == 1) {
                        ssd1306_draw_pixel(x+xo+xx, y+yo+yy, color);
                    } else {
                        ssd1306_fill_rect(x+(xo16+xx)*size, y+(yo16+yy)*size, size, size, color);
                    }
                }
                bits <<= 1;
            }
...
void ssd1306_contrast(uint8_t level)
{
    ssd1306_command(SSD1306_SETPRECHARGE);       // 0xD9
    ssd1306_command(0x00);
    ssd1306_command(SSD1306_SETVCOMDETECT);      // 0xDB
    ssd1306_command(0x00);
    ssd1306_command(SSD1306_SETCONTRAST);        // 0x81
    ssd1306_command(level);
 }

void ssd1306_enable(uint32_t scl, uint32_t sda) {
    ...
    ssd1306_init_i2c(scl, sda);
    ssd1306_begin(SSD1306_SWITCHCAPVCC, SSD1306_I2C_ADDRESS, false);
    ssd1306_clear_display();
    ssd1306_set_rotation(2);
}

void ssd1306_disable(uint32_t scl, uint32_t sda) {
    ssd1306_command(0xFF); 
    ssd1306_command(0xE0); 
    ssd1306_command(0xFF); 
    ssd1306_command(0xD1); 
    ssd1306_command(SSD1306_DISPLAYOFF); 
    ssd1306_command(0x8D);    // disable charge pump
    ssd1306_command(0x10);    // disable charge pump
    ssd1306_command(0xFF); 
    ssd1306_command(0xD0); 
	nrf_delay_ms(10);
	/*
...
void ssd1306_set_font(const GFXfont *f) {
    if(f) {
        if(!gfxFont) {
            cursor_y += 6;
        }
